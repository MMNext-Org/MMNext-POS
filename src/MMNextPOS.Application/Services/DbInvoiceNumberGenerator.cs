...
{
    /// <summary>
    /// MySQL-backed invoice number generator. Reserves the next sequence value
    /// for the current year using <c>INSERT ... ON DUPLICATE KEY UPDATE
    /// LastValue = LastValue + 1</c>. The active <see cref="IUnitOfWork"/>
    /// transaction holds a row lock on the <c>InvoiceSequences</c> row, so two
    /// concurrent sales cannot issue the same number.
    /// </summary>
    public sealed class DbInvoiceNumberGenerator : IInvoiceNumberGenerator
    {
        private readonly IUnitOfWork _unitOfWork;

        public DbInvoiceNumberGenerator(IUnitOfWork unitOfWork)
        {
...
INSERT INTO InvoiceSequences (Year, Prefix, LastValue)
...
SELECT LastValue FROM InvoiceSequences WHERE Year = @Year AND Prefix = @Prefix;";
            ...
            return InvoiceNumberFormat.Format(normalizedPrefix, year, newValue);
        }
    }
}
