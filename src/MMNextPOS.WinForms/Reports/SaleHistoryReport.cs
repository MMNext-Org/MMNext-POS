...
namespace MMNextPOS.WinForms.Reports
{
    ...
    public class SaleHistoryReport : BaseReport
    {
        ...
        {
            _sales = (await _salesService!.GetAllAsync(fromDate, toDate, customerId, status, locationId, cancellationToken) ?? new List<Sale>()).ToList()!;

            // Re-apply the inclusive date range and customer filters defensively.
            _sales = _sales.Where(s => s.SaleDate >= fromDate && s.SaleDate < toDate.Date.AddDays(1)).ToList();
            ...
            foreach (var sale in _sales.OrderBy(s => s.SaleDate))
            {
                ...
                var row = CreateDetailRow(new[]
                {
                    ...
                    "N/A", // Payment method not yet tracked on Sale model
                    sale.Status ?? "Open", // Sale model now has Status field
        ...
        public async Task<byte[]> GenerateSaleHistoryAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            await PopulateAsync(fromDate, toDate, customerId, status, locationId, cancellationToken);
