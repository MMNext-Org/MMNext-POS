...
namespace MMNextPOS.WinForms.Reports
{
    ...
    public class OutstandingReport : BaseReport
    {
        ...
        private void BuildDetail(DetailBand band)
        {
            ...
            var customerTitle = new XRLabel
            {
                Text = "CUSTOMER OUTSTANDING (Receivables)",
            ...
            band.Controls.Add(customerTitle);

            _customerTable = new XRTable
            {
                LocationF = new PointF(0, 30),
                SizeF = new SizeF(727, 120),
                Font = new Font("Segoe UI", 9)
            };
            _customerTable.Rows.Add(CreateHeaderRow(new[] { "Customer", "Invoice #", "Date", "Due Date", "Amount", "Paid", "Balance" }));
            band.Controls.Add(_customerTable);

            // Supplier Outstanding Section
            var supplierTitle = new XRLabel
            {
                Text = "SUPPLIER OUTSTANDING (Payables)",
                Font = new Font("Segoe UI", 11, FontStyle.Bold),
                ForeColor = Color.FromArgb(200, 80, 80),
                LocationF = new PointF(0, 160),
                SizeF = new SizeF(727, 25),
                TextAlignment = TextAlignment.MiddleLeft
            };
            band.Controls.Add(supplierTitle);

            _supplierTable = new XRTable
            {
                LocationF = new PointF(0, 190),
                SizeF = new SizeF(727, 100),
                Font = new Font("Segoe UI", 9)
            };
            _supplierTable.Rows.Add(CreateHeaderRow(new[] { "Supplier", "PO #", "Date", "Due Date", "Amount", "Paid", "Balance" }));
            band.Controls.Add(_supplierTable);
        }
        ...
        /// </summary>
        public async Task PopulateAsync(DateTime asOfDate, int? locationId = null, CancellationToken cancellationToken = default)
        {
            // Get customer outstanding
            _customerOutstandings = (await _outstandingService.GetCustomerOutstandingAsync(asOfDate, cancellationToken)).ToList();
            if (locationId.HasValue)
            {
                // Filter by location would need additional logic
            }

            // Get supplier outstanding
            _supplierOutstandings = (await _outstandingService.GetSupplierOutstandingAsync(asOfDate, cancellationToken)).ToList();
            if (locationId.HasValue)
            {
                // Filter by location would need additional logic
            }
        ...
        {
            await PopulateAsync(asOfDate, locationId, cancellationToken);
