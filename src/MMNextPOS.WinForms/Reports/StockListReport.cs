...
namespace MMNextPOS.WinForms.Reports
{
    ...
    public class StockListReport : BaseReport
    {
        ...
        private void BuildDetail(DetailBand band)
        {
            ...
            _detailTable.Rows.Add(CreateDetailRow(new[] { "Sku", "Name", "Category", "Qty", "Price", "Value", "Min Stock", "Low Stock" }));
        ...
        public async Task PopulateAsync(CancellationToken cancellationToken = default)
        {
            ...
            // Note: Product model has no CategoryId; filtering by category deferred
            // until model supports CategoryId navigation property.

            // Update header (use fixed "All Locations" since Product has no LocationId)
            _reportDateLabel.Text = $"As of: {DateTime.Today:yyyy-MM-dd}";
            _locationLabel.Text = "Location: All Locations";
            _filterLabel.Text = "Filter: All Products";
