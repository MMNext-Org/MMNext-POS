...
{
    /// <summary>
    /// Stock Movement Report - detailed list of stock movements (in/out/adjust/transfer).
    /// </summary>
    public class StockMovementReport : BaseReport
    {
        ...
        private void BuildDetail(DetailBand band)
        {
            ...
            _detailTable.Rows.Add(CreateHeaderRow(new[] { "Date", "Type", "Ref #", "Product", "Location", "Qty In", "Qty Out", "Balance", "Reason" }));
        ...
        private void BuildFooter(ReportFooterBand band)
        {
            ...
            _totalInLabel = new XRLabel { Text = "Total In: ", Font = new Font("Segoe UI", 9), ForeColor = Color.Green };
            _totalOutLabel = new XRLabel { Text = "Total Out: ", Font = new Font("Segoe UI", 9), ForeColor = Color.Red };
            _netMovementLabel = new XRLabel { Text = "Net Movement: ", Font = new Font("Segoe UI", 10, FontStyle.Bold) };
        ...
        {
            // Note: This would need IInventoryService to support filtering
            // For now, we'll get all movements and filter in memory
            _movements = (await _inventoryService.GetStockMovementsAsync(cancellationToken)).ToList();
